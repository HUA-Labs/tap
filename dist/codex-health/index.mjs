...
var DEFAULT_STUCK_TURN_SECONDS = 120;
var DEFAULT_STALE_ACTIVE_TURN_SECONDS = 10 * 60;
...
function toRuntimeHealthStatus(status) {
  if (status === "unavailable") return "adapter-unavailable";
  if (status === "ready" || status === "partial" || status === "stale-owner" || status === "active-turn" || status === "stale-active-turn" || status === "stuck-turn" || status === "degraded" || status === "not-observed" || status === "adapter-unavailable" || status === "unknown") {
    return status;
  }
  return "unknown";
}
function isStuckInProgressConversation(conversation, options = {}) {
  const turn = asRecord(conversation?.lastTurn);
  const stuckTurnSeconds = typeof options.stuckTurnSeconds === "number" ? options.stuckTurnSeconds : DEFAULT_STUCK_TURN_SECONDS;
  return turn?.status === "inProgress" && turn.itemCount === 0 && turn.durationMs === null && turn.finalAssistantStartedAtMs === null && typeof turn.turnStartedAgeSeconds === "number" && turn.turnStartedAgeSeconds >= stuckTurnSeconds;
}
function isStaleActiveInProgressConversation(conversation, options = {}) {
  const turn = asRecord(conversation?.lastTurn);
  const staleActiveTurnSeconds = typeof options.staleActiveTurnSeconds === "number" ? options.staleActiveTurnSeconds : DEFAULT_STALE_ACTIVE_TURN_SECONDS;
  return turn?.status === "inProgress" && typeof turn.turnStartedAgeSeconds === "number" && turn.turnStartedAgeSeconds >= staleActiveTurnSeconds;
}
function deriveRuntimeHealthForBinding(binding, options = {}) {
  const checkedAt = normalizeString(options.checkedAt) ?? (/* @__PURE__ */ new Date()).toISOString();
...
function classifyCodexBindings(bindings, ipc, options = {}) {
  const checkedAt = normalizeString(options.checkedAt) ?? (/* @__PURE__ */ new Date()).toISOString();
  const adapter = normalizeString(options.adapter) ?? "codex-desktop-ipc";
  if (!ipc?.supported || !ipc?.connected) {
    return bindings.map((binding) => {
      ...
      const classified = {
        ...
        runtimeHealthStatus: extensionUnknown ? "unknown" : "adapter-unavailable",
        message: extensionUnknown ? "Codex extension MCP presence is visible, but no IPC observe surface is available." : ipc?.supported ? normalizeString(ipc.error) ?? "Codex IPC observe is unavailable." : "Codex IPC observe is not supported on this host.",
  ...
  const conversations = Array.isArray(ipc.conversations) ? ipc.conversations : [];
  const byConversation = new Map(
    conversations.map((entry) => [entry.conversationId, entry])
  );
  return bindings.map((binding) => {
    const live = binding.conversationId ? byConversation.get(binding.conversationId) : null;
    let lifecycleStatus = "unavailable";
    let runtimeHealthStatus = "adapter-unavailable";
    ...
    if (binding.conversationId && !binding.ownerClientId) {
      ...
      message = "No live IPC conversation was observed for this conversationId.";
    } else if (binding.conversationId && binding.ownerClientId && live?.ownerClientId === binding.ownerClientId) {
      ...
      if (isStuckInProgressConversation(live, options)) {
        ...
        message = "Live Codex conversation has an in-progress turn with no emitted items.";
      } else if (isStaleActiveInProgressConversation(live, options)) {
        runtimeHealthStatus = "stale-active-turn";
        message = "Live Codex conversation has an old in-progress turn; the UI may be idle but IPC still reports it active.";
