...
mcp.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  if (req.params.name === "tap_who") {
    ...
    const zombieThreshold = 10 * 60 * 1000;
    const store = loadHeartbeats();
    const agents: Array<{
      agent: string;
      status: string;
    ...
    for (const [name, hb] of Object.entries(store)) {
      ...
      if (hbTime < cutoff) continue;
      const actTime = new Date(hb.lastActivity).getTime();
      const zombie =
        hb.status === "active" && Date.now() - actTime > zombieThreshold;
      agents.push({
        agent: name,
        status: zombie ? "zombie" : hb.status,
      ...
      content: [
        {
          type: "text",
          text: dryRun
            ? `[DRY RUN] Would archive ${moved.length} files older than ${days} days.`
            : `Archived ${moved.length} files older than ${days} days to archive/.`,
        },
      ],
    };
  }
