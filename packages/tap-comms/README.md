...
> **v1 note**: Currently requires a monorepo with `packages/tap-plugin/channels/` present. External standalone usage is planned for a future release.

```bash
# 1. Initialize comms directory and state
npx @hua-labs/tap init

# 2. Add runtimes (run from monorepo root)
...
Requires `bun` and local `tap-plugin/channels` (v1).
