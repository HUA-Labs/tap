...
#   3. Verify or sync tracked .claude/settings.json
...
step "3/5 Verify tracked settings.json"
SETTINGS_SRC="${MAIN_DIR}/.claude/settings.json"
if [[ -f "$SETTINGS_SRC" ]]; then
  mkdir -p "$WORKTREE_PATH/.claude"
  cp "$SETTINGS_SRC" "$WORKTREE_PATH/.claude/settings.json"
  ok "Tracked settings.json synchronized"
else
  warn ".claude/settings.json not found in main repo — agent sessions may require manual permission grants"
fi
