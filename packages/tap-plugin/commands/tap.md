...
**Step 5: Verify tracked settings.**

Each worktree should inherit the tracked team SSOT `.claude/settings.json`:

```bash
for wt in "$WT_PATH"; do
  test -f "$wt/.claude/settings.json" || cp "${REPO_ROOT}/.claude/settings.json" "$wt/.claude/settings.json"
done
```
