...
- Added a generic profile-pack example and preserved the existing narrative
  examples with a guard that they are not copy/paste setup defaults.
...
- `npm pack --pack-destination /tmp/tap-standalone-0.6-pack-r2 --json` passed
  and produced `hua-labs-tap-0.6.0.tgz` with 53 entries.
- Tarball audit passed: required docs/bin/dist/example entries present, 16
  public docs/examples checked, source-map `sourcesContent` count 0, and
  private/secret pattern matches 0.
